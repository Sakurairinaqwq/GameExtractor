...
**********************************************************************************************
**/
public class Viewer_TRM_TRM extends ViewerPlugin {
  ...
  **********************************************************************************************
  **/
  public Viewer_TRM_TRM() {
    super("TRM_TRM", "TRM 3D Model");
    setExtensions("trm");

    setGames("Tomb Raider I-III Remastered");
    setPlatforms("PC");
  }
  ...
  **/
  ...
  public PreviewPanel read(FileManipulator fm) {
    try {
      ...
      // 4 - Number of Shaders (1)
      int numShaders = fm.readInt();
      FieldValidator.checkNumFiles(numShaders + 1); // +1 to allow nulls, just in case

      // X - Shaders
      fm.skip(numShaders * 44);

      // 4 - Number of Textures
      int numTextures = fm.readInt();
      FieldValidator.checkNumFiles(numTextures + 1); // +1 to allow nulls, just in case

      // X - Textures (and padding to 4-byte multiple)
      int textureLength = numTextures * 2;
      textureLength += ArchivePlugin.calculatePadding(textureLength, 4);
      fm.skip(textureLength);

      // 4 - Number of Animations
      int numAnimations = fm.readInt();
      FieldValidator.checkNumFiles(numAnimations + 1); // +1 to allow nulls, just in case

      if (numAnimations != 0) {
        // for each Animation
        //   48 - Unknown
        fm.skip(numAnimations * 48);

        // 4 - Number of Entries 2
        int numEntries = fm.readInt();
        FieldValidator.checkNumFiles(numEntries + 1); // +1 to allow nulls, just in case
        ...
        // 2 - Number of Entries 4
        // 2 - Unknown
        int numEntries4 = fm.readShort();
        fm.skip(2);
        FieldValidator.checkNumFiles(numEntries4 + 1); // +1 to allow nulls, just in case

        // for each Entry3 x Entry4
        //   48 - Unknown
        fm.skip(numEntries * numEntries4 * 48);
      }
