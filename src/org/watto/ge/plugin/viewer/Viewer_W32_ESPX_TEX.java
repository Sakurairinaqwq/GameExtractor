...
**********************************************************************************************
**/
public class Viewer_W32_ESPX_TEX extends ViewerPlugin {
  ...
  **********************************************************************************************
  **/
  public Viewer_W32_ESPX_TEX() {
    super("W32_ESPX_TEX", "W32_ESPX_TEX Image");
    setExtensions("tex");
  ...
  **/
  ...
  public int getMatchRating(FileManipulator fm) {
    try {
      ...
      if (plugin instanceof Plugin_W32_ESPX) {
        rating += 50;
      }
  ...
  **/
  ...
  public ImageResource readThumbnail(FileManipulator fm) {
    try {
      ...
      // 4 - Image Data Offset (pointer in to the archive itself, not into this file specifically)
      int offset = fm.readInt();

      File archiveFile = Archive.getBasePath();
      FieldValidator.checkOffset(offset, archiveFile.length());

      // Open the Archive File and use that for reading the pixel data

      FileManipulator imageFM = new FileManipulator(archiveFile, false);
      imageFM.seek(offset);
      ...
      if (imageFormat == 12) {
        ...
        imageResource = ImageFormatReader.readDXT3(imageFM, width, height);
      }
      else if (imageFormat == 15) {
        imageResource = ImageFormatReader.readDXT5(imageFM, width, height);
      }
      else if (imageFormat == 18) {
        imageResource = ImageFormatReader.readRGBA(imageFM, width, height);
      }
      else if (imageFormat == 19) {
        imageResource = ImageFormatReader.read8BitPaletted(imageFM, width, height);
      }
      else {
        ErrorLogger.log("[Viewer_W32_ESPX_TEX] Unknown Image Format: " + imageFormat);
        imageFM.close();
        return null;
      }
