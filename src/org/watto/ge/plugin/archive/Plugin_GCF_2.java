...
**********************************************************************************************
**/
public class Plugin_GCF_2 extends ArchivePlugin {
  ...
  **/
  public Plugin_GCF_2() {
    ...
    setGames("Dark Messiah of Might and Magic");
    setExtensions("gcf"); // MUST BE LOWER CASE
    setPlatforms("PC");

    // MUST BE LOWER CASE !!!
    setFileTypes(new FileType("vtf", "VTF Image", FileType.TYPE_IMAGE));

    setTextPreviewExtensions("vmt", "dsp", "dump", "gam", "lst", "prj", "rc", "res", "scr", "vas", "vcd", "vdf", "ain", "qc", "qct"); // LOWER CASE
  ...
  public Resource[] read(File path) {
    try {
      ...
      for (int i = 0; i < numBlocks; i++) {
        ...
        // 4 - Next Data Block Index (-1 or 65535 if there is no next index)
        int nextDataBlockIndex = fm.readInt();
        if (nextDataBlockIndex != -1 && nextDataBlockIndex != 65535) {
      ...
      for (int i = 0; i < numEntries; i++) {
        ...
        if (entryType == 0) {
          // Folder
          FieldValidator.checkNumFiles(length + 1); // +1 to allow folders with no files in them
        }
        else {
          // File
          ...
          if (blockIndex == -1) {
            ...
            //if (names[i].equals("\\materials\\models\\books\\book03.vtf")) {
            //  System.out.println("here");
            //}

            // now, not all the blocks are sequential, so we need to build them up for each file
            // each block should be size paddingMultiple except the last one
            int numFileBlocks = length / paddingMultiple;
            int lastBlockLength = length % paddingMultiple;
            if (lastBlockLength != 0) {
              numFileBlocks++;
            }
            long[] fileBlockOffsets = new long[numFileBlocks];
            long[] fileBlockLengths = new long[numFileBlocks];

            int nextIndex = firstDataBlockIndexes[blockIndex];
            for (int b = 0; b < numFileBlocks; b++) {
              fileBlockOffsets[b] = (nextIndex * paddingMultiple) + relOffset;
              fileBlockLengths[b] = paddingMultiple;
              nextIndex = nextDataBlockIndexes[nextIndex];
            }
            if (lastBlockLength != 0) {
              fileBlockLengths[numFileBlocks - 1] = lastBlockLength;
            }

            ExporterPlugin blockExporter = new BlockExporterWrapper(exporterDefault, fileBlockOffsets, fileBlockLengths, fileBlockLengths);

            //path,name,offset,length,decompLength,exporter
            resources[realNumFiles] = new Resource(path, names[i], offset, length, length, blockExporter);
            realNumFiles++;
