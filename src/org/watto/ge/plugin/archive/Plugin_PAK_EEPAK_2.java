...
**********************************************************************************************
**/
public class Plugin_PAK_EEPAK_2 extends ArchivePlugin {
  ...
  **********************************************************************************************
  **/
  public Plugin_PAK_EEPAK_2() {

    super("PAK_EEPAK_2", "PAK_EEPAK_2");

    //         read write replace rename
    setProperties(true, false, false, false);

    setGames("Cake Shop 3");
  ...
  public void readDirectory(File path, FileManipulator fm, Resource[] resources, String dirName, long arcSize) {
    try {
      ...
      for (int i = 0; i < numFilesInFolder; i++) {
        // 1 - Filename Length (number of unicode characters) (including 2 bytes for null terminator)
        int filenameLength = ByteConverter.unsign(fm.readByte());

        // X - Filename (unicode)
        // 2 - null Unicode Filename Terminator
        String filename = dirName + fm.readNullUnicodeString(filenameLength);
      ...
      for (int i = 0; i < numFolders; i++) {
        // 1 - Folder Name Length (number of unicode characters) (including 2 bytes for null terminator)
        int folderNameLength = ByteConverter.unsign(fm.readByte());

        // X - Folder Name (unicode)
        // 2 - null Unicode Folder Name Terminator
        String folderName = dirName + fm.readNullUnicodeString(folderNameLength) + "\\";
